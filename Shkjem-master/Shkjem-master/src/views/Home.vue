  ...
  <div class="home" v-loading="loading">
    <swiper id="swiperBox" v-bind:options="swiperOption" ref="mySwiper">
      <swiper-slide class="swiper-slide slide-one">
        <div class="page">
          <h3>思政教育</h3>
          <p>Ideological and Political education</p>
        </div>
        ...
        <ul class="case-item" >
          ...
          >
            <router-link 
              ...
              :to="{ name: 'ClassDails', params: { id: item.id }}"
            >
      ...
      <swiper-slide class="swiper-slide slide-three">
        ...
        <div class="news-content">
          <div class="news-content-item" v-for="(news,i) in newsList" :key="i" @click="Addview_news(news.id)">
            <div :style="'order: '+ (i%2==0 ? 1: 3)" >
              <router-link
                class="text-decoration"
                :to="{ name: 'newsdetails', params: { id: news.id, model: 'swiper' }}"
              >
                <div class="item-img" v-lazy:background-image="news.img"></div>
              </router-link>
            </div>
            <div style="order: 2">
              <el-divider>
...
export default {
  ...
  methods: {
    //下面两个是增加浏览次数的方法
    AddViews(id){
      this.$http.get("/api/Cases/View?id="+id)
    },
    Addview_news(id){
      this.$http.get("/api/news/View?id="+id)
    }
  },
  mounted() {
    localStorage.setItem("video-type","0")
    this.$http   //与后端进行交互数据
      .all([
        this.$http.get("/api/Cases/GetCasesAll"),
        this.$http.get("/api/Cases/getAll")
      ])
      .then(
